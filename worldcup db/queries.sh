...
echo "$($PSQL "SELECT COUNT(*) AS games_with_more_than_2_wg FROM games WHERE winner_goals>2;")"
...
echo "$($PSQL "SELECT name FROM games AS g INNER JOIN teams AS t ON g.winner_id=t.team_id OR g.opponent_id=t.team_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name;")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM games AS g INNER JOIN teams AS t ON g.winner_id=t.team_id OR g.opponent_id=t.team_id WHERE name LIKE 'Co%' ORDER BY name;")"
